...
function dashBoardRender(){
  ...
  let totalmembers=document.getElementById('members')
   let issuedBooksTag=document.getElementById('issuedBooks')
  let books=JSON.parse(localStorage.getItem('books'))
  let members=JSON.parse(localStorage.getItem('users'))

  let issuesbooks=books.filter(function(book){
    return book.status=="Issued"
  })
  totalBooks.innerText=`${books.length}`
  totalmembers.innerText=members.length
  issuedBooksTag.innerText=issuesbooks.length
